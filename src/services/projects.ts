...
export const projectsApi = {
	...
	createMessage: (id: string, input: { content: string; replaceMessageId?: string; generation?: { kind?: "melody" | "chords" | "chords_and_melody" | "counter_melody" | "bassline" | "drums" | "full_composition"; key?: string; scale?: "major" | "minor"; tempo?: number; mood?: string; lengthBars?: number; complexity?: "low" | "medium" | "high"; variationAmount?: number; timeSignature?: [number, number] } }, options?: RequestInit) => request<{ data: ProjectConversationResult }>(`/projects/${encodeURIComponent(id)}/messages`, { method: "POST", body: JSON.stringify(input), ...options }),
	refine: (id: string, input: { prompt: string; kind?: string }, options?: RequestInit) => request<{ data: PromptRefinementResult }>(`/projects/${encodeURIComponent(id)}/refine`, { method: "POST", body: JSON.stringify(input), ...options }),
};
