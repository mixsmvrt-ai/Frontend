...
function CreditUsage({ membership }: { membership: MembershipSnapshot | null }) {
  ...
  const allocation = credits?.monthlyAllocation ?? (membership?.plan === "plus" ? 3000 : 1500);
  const used = credits?.used ?? 0;
  const percent = Math.min(100, allocation ? (used / allocation) * 100 : 0);
