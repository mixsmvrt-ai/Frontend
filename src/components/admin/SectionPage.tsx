...
const chartData = [{ name: "Jan 1", value: 34, secondary: 20 }, { name: "Jan 8", value: 48, secondary: 28 }, { name: "Jan 15", value: 42, secondary: 34 }, { name: "Jan 22", value: 67, secondary: 39 }, { name: "Jan 29", value: 58, secondary: 47 }, { name: "Feb 5", value: 76, secondary: 52 }, { name: "Feb 12", value: 84, secondary: 61 }];
...
const resourceByTitle: Record<string, string> = { Payments: "payments", Subscriptions: "payments", Projects: "projects", "MIDI Library": "midi-library", "AI Generation": "generations", Support: "supportTickets", Settings: "settings", "Music Brain": "artistProfiles", Referrals: "referrals" };
const formatCell = (value: unknown) => value === null || value === undefined || value === "" ? "-" : typeof value === "object" ? JSON.stringify(value) : String(value);
const fieldAliases: Record<string, string> = { Name: "display_name", User: "display_name", Customer: "display_name", Owner: "display_name", Profile: "artist_name", "Pack name": "name", Project: "name", Subject: "subject", "Transaction ID": "id", "Plan": "membership_type", Status: "membership_status", Created: "created_at", "Last updated": "updated_at", "Last active": "last_active_at" };
...
export default function SectionPage({ config }: { config: Config }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (records.length) {
          setRows(records.map((rawRecord) => {
            const record = rawRecord as Record<string, unknown>;
            return config.columns.map((column) => {
            const key = fieldAliases[column] ?? column.toLowerCase().replaceAll(" ", "_");
            return formatCell(record[key] ?? record[column] ?? record.id);
            });
          }));
        }
      } catch {
        // Keep the initial shell data visible when a live resource is unavailable.
      } finally {
        setLoading(false);
      }
    };
  ...
  return <div className="mx-auto max-w-[1500px]">
    ...
    <div className="grid grid-cols-2 gap-3 xl:grid-cols-4 sm:gap-4">{config.metrics.map((metric, index) => <MetricCard key={metric.label} {...metric} icon={[SlidersHorizontal, Filter, Download, Plus][index]} />)}</div>
