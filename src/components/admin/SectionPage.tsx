...
export default function SectionPage({ config }: { config: Config }) {
  ...
  const tabResources: Record<string, string> = { Genres: "genreProfiles", Artists: "artistProfiles", "Tempo profiles": "tempoRanges", Scales: "scales", "Chord libraries": "genreChords", "Instrument profiles": "instrumentCategories", Rules: "musicRules" };
  const resource = config.title === "Music Brain" ? tabResources[tab] ?? "artistProfiles" : config.resource ?? resourceByTitle[config.title];
  ...
  const tabFilteredRecords = records.filter((record) => {
    const selectedTab = tab.toLowerCase();
    if (selectedTab.includes("all") || selectedTab.includes("genres") || selectedTab.includes("artists") || selectedTab.includes("tempo") || selectedTab.includes("scales") || selectedTab.includes("chord") || selectedTab.includes("instrument") || selectedTab.includes("rules")) return true;
    const status = String(record.status ?? record.membership_status ?? "").toLowerCase();
    if (selectedTab.includes("trial")) return status.includes("trial");
    if (selectedTab.includes("paid") || selectedTab.includes("pro")) return status.includes("pro") || String(record.membership_type ?? "").toLowerCase() === "pro";
    if (selectedTab.includes("cancel")) return status.includes("cancel") || Boolean(record.cancel_at_period_end);
    if (selectedTab.includes("suspend") || selectedTab.includes("ban")) return status.includes("suspend") || status.includes("ban");
    if (selectedTab.includes("admin")) return String(record.user_roles ?? "").toLowerCase().includes("admin");
    if (selectedTab.includes("pending") || selectedTab.includes("review")) return status.includes("pending") || status.includes("review");
    return true;
  });
  const rows = tabFilteredRecords.map((record) => config.columns.map((column, index) => valueForColumn(record, column, index)));
  const visibleRows = rows.filter((row) => row.join(" ").toLowerCase().includes(query.toLowerCase()));
  const visibleRecords = tabFilteredRecords.filter((record) => config.columns.map((column, index) => valueForColumn(record, column, index)).join(" ").toLowerCase().includes(query.toLowerCase()));
  ...
  const openRecord = (record: Record<string, unknown>) => { setSelected(record); setEditValue(String(record.status ?? record.membership_status ?? "")); };
  const saveStatus = async () => { if (!selected || !resource || !selected.id || !editValue) return; setActionBusy(true); try { await updateAdminResource(resource, String(selected.id), { ...(config.title === "Users" ? { membershipStatus: editValue } : { status: editValue }) }); setRecords((current) => current.map((record) => record.id === selected.id ? { ...record, ...(config.title === "Users" ? { membership_status: editValue } : { status: editValue }) } : record)); setSelected(null); } finally { setActionBusy(false); } };
  const runUserAction = async (action: "temporary_password" | "credits" | "plan" | "ban" | "unban") => { if (!selected?.id) return; const value = window.prompt(action === "temporary_password" ? "Temporary password (8+ characters)" : action === "credits" ? "Credits to grant" : action === "plan" ? "Plan: go or plus" : action === "ban" ? "Ban duration in days" : "Confirm unban", action === "unban" ? "" : undefined); if (value === null) return; setActionBusy(true); try { await runAdminUserAction(String(selected.id), action === "temporary_password" ? { action, password: value } : action === "credits" ? { action, credits: Number(value) } : action === "plan" ? { action, plan: value.toLowerCase() === "go" ? "go" : "plus" } : action === "ban" ? { action, banDays: Number(value) } : { action }); setSelected(null); } finally { setActionBusy(false); } };
  return <div className="mx-auto max-w-[1500px]">
    ...
    <section className="mt-4 rounded-xl border border-white/[0.07] bg-[#181822] shadow-[0_18px_50px_rgba(0,0,0,.14)]"><div className="flex flex-col gap-3 border-b border-white/[0.07] p-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex min-w-0 gap-1 overflow-x-auto overscroll-x-contain scrollbar-hidden">{config.tabs.map((item) => <button key={item} onClick={() => { setTab(item); setQuery(""); }} className={`whitespace-nowrap rounded-md px-3 py-2 text-xs font-medium transition ${tab === item ? "bg-violet-500/15 text-violet-300" : "text-slate-500 hover:text-slate-200"}`}>{item}</button>)}</div><div className="flex shrink-0 gap-2"><label className="flex h-9 min-w-48 items-center gap-2 rounded-lg border border-white/[0.08] bg-white/[0.02] px-3"><Search size={14} className="text-slate-600" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search records..." className="w-full bg-transparent text-xs text-slate-200 outline-none placeholder:text-slate-600" /></label><button className="grid size-9 place-items-center rounded-lg border border-white/[0.08] text-slate-500 hover:text-slate-200" aria-label="Filter records"><Filter size={15} /></button></div></div><div className="max-h-[60vh] overflow-auto overscroll-contain scrollbar-hidden"><table className="w-full min-w-[760px] text-left"><thead className="sticky top-0 z-10 bg-[#211b2f]"><tr>{config.columns.map((column) => <th key={column} className="px-5 py-3 text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-600">{column}</th>)}<th className="px-5 py-3" /></tr></thead><tbody>{visibleRows.map((row, index) => <tr key={`${row[0]}-${index}`} className="border-t border-white/[0.05] transition hover:bg-white/[0.025]"><>{row.map((cell, cellIndex) => <td key={`${cell}-${cellIndex}`} className={`px-5 py-4 text-xs ${cellIndex === 0 ? "font-medium text-slate-200" : "text-slate-500"}`}>{cellIndex === 0 && config.title === "Users" ? <span className="flex items-center gap-2.5"><span className="grid size-7 place-items-center rounded-full bg-violet-500/20 text-[10px] font-semibold text-violet-200">{cell.split(" ").map((part) => part[0]).join("").slice(0, 2)}</span>{cell}</span> : cell}</td>)}</><td className="px-5 py-4 text-right"><button onClick={() => openRecord(visibleRecords[index] ?? {})} className="rounded-md p-1.5 text-slate-600 hover:bg-white/[0.06] hover:text-slate-200" aria-label={`Open ${row[0]}`}><MoreHorizontal size={16} /></button></td></tr>)}</tbody></table></div><div className="flex items-center justify-between border-t border-white/[0.07] px-5 py-3 text-[11px] text-slate-600"><span>{loading ? "Loading live records..." : `Showing ${visibleRows.length} of ${rows.length} records`}</span><span>Live admin data</span></div></section>
    {selected && <><button className="fixed inset-0 z-40 bg-black/60" onClick={() => setSelected(null)} aria-label="Close detail drawer" /><aside className="fixed inset-y-0 right-0 z-50 w-full max-w-md overflow-y-auto border-l border-white/[0.08] bg-[#111118] p-6 shadow-2xl"><div className="flex items-start justify-between"><div><p className="text-xs text-violet-400">Record details</p><h3 className="mt-2 text-xl font-semibold text-white">{formatCell(selected[config.columns[0] === "Name" ? "display_name" : config.columns[0].toLowerCase().replaceAll(" ", "_")])}</h3></div><button onClick={() => setSelected(null)} className="grid size-8 place-items-center rounded-lg border border-white/[0.08] text-slate-500 hover:text-slate-200" aria-label="Close detail drawer"><X size={16} /></button></div><div className="mt-8 space-y-4">{Object.entries(selected).filter(([key]) => !["id", "created_at", "updated_at"].includes(key)).slice(0, 12).map(([key, value]) => <div key={key} className="flex items-start justify-between gap-4 border-b border-white/[0.06] pb-3"><span className="text-xs text-slate-600">{key}</span><span className="max-w-[65%] text-right text-xs font-medium text-slate-200">{formatCell(value)}</span></div>)}</div>{config.title === "Users" ? <div className="mt-8 grid grid-cols-2 gap-2"><button disabled={actionBusy} onClick={() => void runUserAction("temporary_password")} className="rounded-lg bg-violet-600 px-3 py-2.5 text-xs font-semibold text-white disabled:opacity-50">Temp password</button><button disabled={actionBusy} onClick={() => void runUserAction("credits")} className="rounded-lg bg-emerald-600 px-3 py-2.5 text-xs font-semibold text-white disabled:opacity-50">Grant credits</button><button disabled={actionBusy} onClick={() => void runUserAction("ban")} className="rounded-lg bg-red-600 px-3 py-2.5 text-xs font-semibold text-white disabled:opacity-50">Ban user</button><button disabled={actionBusy} onClick={() => void runUserAction("unban")} className="rounded-lg border border-white/10 px-3 py-2.5 text-xs font-medium text-slate-300 disabled:opacity-50">Unban user</button></div> : null}<div className="mt-8 flex gap-2"><input value={editValue} onChange={(event) => setEditValue(event.target.value)} placeholder="Status" className="min-w-0 flex-1 rounded-lg border border-white/10 bg-white/[.04] px-3 py-2 text-xs text-white" /><button disabled={actionBusy || !editValue} onClick={() => void saveStatus()} className="rounded-lg bg-violet-600 px-3 py-2.5 text-xs font-semibold text-white disabled:opacity-50">Save status</button></div></aside></>}
  </div>;
}
