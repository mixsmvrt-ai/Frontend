...
export default function SectionPage({ config }: { config: Config }) {
  ...
  const saveStatus = async () => { if (!selected || !resource || !selected.id || !editValue) return; setActionBusy(true); try { await updateAdminResource(resource, String(selected.id), { status: editValue }); setRecords((current) => current.map((record) => record.id === selected.id ? { ...record, status: editValue, membership_status: editValue } : record)); setSelected(null); } finally { setActionBusy(false); } };
