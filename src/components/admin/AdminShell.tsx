...
export default function AdminShell({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  return (
    <div data-admin-shell className="min-h-screen bg-[#0B0B0F] text-slate-100">
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-64 flex-col border-r border-white/[0.07] bg-[#111118] px-3 py-5 transition-transform duration-200 lg:translate-x-0 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
      ...
      <div className="lg:pl-64">
        ...
        <main className="min-h-[calc(100vh-72px)] px-4 py-6 sm:px-7 sm:py-8">{children}</main>
      </div>
    </div>
  );
}
