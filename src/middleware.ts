...
export async function middleware(request: NextRequest) {
  ...
  const supabase = createServerClient(url, key, { cookies: { getAll: () => request.cookies.getAll(), setAll: (cookies: SupabaseCookie[]) => cookies.forEach(({ name, value, options }: SupabaseCookie) => response.cookies.set(name, value, options)) } });
